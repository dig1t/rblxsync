...
| `rblxsync import [--universe-id ID] [--place-id ID]… [--badge-id ID]…` | Pull a live experience's metadata **down** into `rblxsync.yml` + `rblxsync-lock.yml`. For adopting rblxsync on an existing game, or absorbing a resource it doesn't know about yet. **Destructive on conflicts** (remote wins); keeps local-only entries; drops stale lock entries; writes stable `id`s. Backs up the old yml to `rblxsync.old.yml` (then `.old1`…). Icons are NOT imported. The API key only auto-discovers the root place — pass `--place-id` (repeatable) for others. **Disabled badges aren't listed by Roblox** — pass `--badge-id` (repeatable) to import each by id. `--place-id`/`--badge-id` are validated against the experience id. |
